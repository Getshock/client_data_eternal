...
local function updateLabelText()
    if enterGame:getChildById('clientComboBox') and tonumber(enterGame:getChildById('clientComboBox'):getText()) > 1080 then
        ...
        enterGame:getChildById('rememberEmailBox'):setText("Remember Login")
    end
end
...
function EnterGame.tryHttpLogin(clientVersion, httpLogin)
    ...
    if not modules.game_things.isLoaded() then
        ...
        local errorBox = displayErrorBox(tr("Login Error"),
            string.format("Things are not loaded, please put assets in things/%d/<assets>.", clientVersion))
        connect(errorBox, {
            onOk = EnterGame.show
        })
        return
...
function EnterGame.doLogin()
    ...
    if clientVersion >= 1281 and G.port ~= 7171 then
        ...
        if modules.game_things.isLoaded() then
            ...
            local errorBox = displayErrorBox(tr("Login Error"),
                string.format("Things are not loaded, please put spr and dat in things/%d/<here>.", clientVersion))
            connect(errorBox, {
                onOk = EnterGame.show
            })
            return
